...
{
    public class AuctionStartBackgroundJob : AsyncBackgroundJob<ProductDto>, ITransientDependency
    {
        ...
        public AuctionStartBackgroundJob(IUnitOfWorkManager unitOfWorkManager, IBackgroundJobManager backgroundJobManager, IRepository<Product.Product, long> productRepository)
        {
        ...
        public override async Task ExecuteAsync(ProductDto record)
        {
            ...
            {
                AsyncHelper.RunSync(() => StartAuction(tenant.Id, record));
            }
        }
        public async Task StartAuction(int tenantId, ProductDto product)
        {
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                var record = await _productRepository.FirstOrDefaultAsync(x => x.Id == product.Id);
                record.Status = ProductStatus.InAuction;
